...
import { useGetSizesQuery } from '../../../../redux/features/authApiSlice';
...
const SizesComponent = () => {
  ...
  // Fetch Size from Redux slice
  const { data, isLoading, error } = useGetSizesQuery({ page, page_size: pageSize });
  ...
  // Navigate to view Size details dynamically
  const handleViewSize = (id: number) => {
    router.push(`/core/core-size/${id}`);
  };

  if (isLoading) return <p>Loading Size...</p>;
  if (error) return <p>Error loading Size.</p>;

  // Filter size based on search query
  const filteredSizes= data?.results?.filter((size: Size) =>
  ...
  return (
    <div>
      ...
      {/* Table displaying filtered Size */}
        ...
        <table className="w-full table-auto">
          <thead>
            <tr className="bg-gray-2 text-center dark:bg-meta-4">
              ...
              <th className="min-w-[140px] px-5 py-4 text-sm text-black dark:text-white">Size Name</th>
          ...
          <tbody>
            {filteredSizes?.map((item, rowIndex) => (
              <tr key={item.id} className="border-b border-[#eee] dark:border-strokedark">
                ...
                <td className="px-4 py-5 text-center">
                  <span className="text-black dark:text-white">{item.size_name}</span>
                </td>
                <td className="px-4 py-5 text-center">
                ...
                <td className="px-3 py-5 text-center">
                  <div className='space-x-3'>
                    <button
                      ...
                      onClick={() => handleViewSize(item.id)}
                    >
                      View
                    </button>
                    <button
      ...
      {/* Add Sizes Modal */}
      <AddSizeModal isOpen={isModalOpen} onClose={closeModal} />
